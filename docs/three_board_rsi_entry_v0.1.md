...
同一股票出现新周期时，逐日重放会在新周期 `sequence_start_date` 当天先终止
此前最新且仍开放的旧周期。旧周期设置
`invalid_reason=SUPERSEDED_BY_NEW_CYCLE`、`current_stage=INVALID`，并以
`superseded_date` 明确记录终止日；该日及以后不再更新旧周期的 MD_1/MD_2
状态或产生新信号。旧周期在终止日前已经产生的历史信号原样保留，新周期的准入、
指标状态、信号状态和 20 日窗口均独立计算。自然到期、已因 RSI 跌破 60
完全失效或此前已被替代的周期不会被重复标记。
...
旧周期替代同样按此逐日顺序发生：在新周期首条人工记录进入重放前，旧周期不会
提前知道未来会被替代；只有重放到新周期 `sequence_start_date` 当天才写入
`superseded_date` 并停止旧周期。
